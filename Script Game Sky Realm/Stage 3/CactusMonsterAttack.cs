...
public class CactusMonsterAttack : MonoBehaviour {
	...
	void Update () {
		...
		enemyDeathStatus = this.GetComponent<CactusMonsterHealth> ().enemyDeathStatus;
		...
		if (flagToAttack == true && enemyDeathStatus == false) {
			myAnimator.SetTrigger ("Power Punch Attack");
		}

		if (Input.GetKeyDown (KeyCode.C)) 
		{
			myAnimator.SetTrigger ("Soundwave Attack");
		}
		//
	...
	void CastSoundWave()
	{
		GameObject bullet2 = (GameObject)Instantiate (soundWave, new Vector3(transform.position.x, transform.position.y +1, transform.position.z), Quaternion.identity);
		bullet2.GetComponent<Rigidbody>().AddForce (transform.forward * 5, ForceMode.Impulse);
	}
	...
	void SpawnBullet()
	{
		GameObject bullet2 = (GameObject)Instantiate (bullet, transform.position, Quaternion.identity);
	...
	void MeleeAttack()
	{
		...
		*/
		...
		if (Vector3.Dot (toOther, transform.forward) > 0.1f && distanceToPlayer <5f) {
			Debug.Log ("hit player");
	...
	void WalkingToPlayer()
	{
		...
		if (distanceToPlayer >5 && enemyDeathStatus == false && agent.enabled == true) {
			flagToAttack = false;
