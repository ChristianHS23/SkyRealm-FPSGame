...
public class CactusMonsterHealth : MonoBehaviour {
	...
	void Update () {
		...
		if (health <= 0&&enemyDeathStatus==false) {
			...
			//stage2Manager.GetComponent<Stage2Manager> ().deathCount += 1;
			player.GetComponent<PlayerInteraction2> ().S2EnemyKillCount += 1;//ini flag buat itung death count
